...
document.getElementById("set-visual-profile")?.addEventListener("change", (e) => {
  const s = loadSettings();
  const profile = normalizeVisualProfile(e.target.value);
  Object.assign(s, visualProfileSettings(profile), { visualProfile: profile });
  saveSettings(s);
  applyVisualSettingsLive(s);
  showToast(`Visual profile: ${profile}`);
});
