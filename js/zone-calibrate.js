...
export function isCalibrating() { return calibrating; }

export function toggleCalibrateMode() {
  console.log("Calibration mode:" + (!calibrating ? "ENTERING" : "EXITING"));
  calibrating = !calibrating;
  if (calibrating) {
    showStatus('CALIBRATE: drag zone markers to correct positions. Press C to save.');
...
function showStatus(msg) {
  if (!statusEl) {
    ...
    statusEl.style.cssText = 'position:fixed;top:12px;left:50%;transform:translateX(-50%);z-index:30;color:#0f0;font:13px monospace;background:rgba(0,0,0,0.8);padding:6px 16px;border-radius:4px;border:1px solid rgba(0,255,0,0.3);pointer-events:none';
...
function showCoords(x, z) {
  if (!coordEl) {
    ...
    coordEl.style.cssText = 'position:fixed;bottom:40px;left:50%;transform:translateX(-50%);z-index:30;color:#ff0;font:12px monospace;background:rgba(0,0,0,0.8);padding:4px 12px;border-radius:3px;pointer-events:none';
...
function onMouseDown(e) {
  ...
  // Update mouse coords from event before raycasting
  const canvas = renderer.domElement;
  const rect = canvas.getBoundingClientRect();
  ...
  if (hits.length > 0) {
    ...
    selectedSprite.material.opacity = 0.6;
...
function savePositions() {
  ...
  try {
    ...
    showStatus('SAVED: ' + Object.keys(data).length + ' zone positions. Press C to exit calibration.');
    setTimeout(function() { if (!calibrating && statusEl) statusEl.style.display = 'none'; }, 2000);
  } catch (e) {
