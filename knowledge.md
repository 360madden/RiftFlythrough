...
8. `capture_visual_baselines.py` is the fixed-camera Beauty-profile benchmark helper: it hides viewer chrome, applies deterministic world-relative camera poses, and captures comparable scene artifacts for visual-fidelity review.
...
- **Fixed-camera visual baselines:** `capture_visual_baselines.py` starts the viewer with the Beauty profile, hides UI chrome, applies deterministic world-relative camera poses (`overview`, `north-low`, `east-oblique`, `south-detail`), captures viewport/canvas PNGs under ignored `artifacts/visual-baselines/`, and writes `visual-baselines-report.json` with pose, timing, artifact hash, and failure details.
